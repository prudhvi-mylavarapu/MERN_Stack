...
const Navbar = ({activeItem, setActiveItem}) => {
    ...
    return (
        <nav className='sticky top-0 p-4 bg-white z-50 shadow-sm px-4 sm:px-6 md:px-10 lg:px-20 xl:px-40'>
            <ul className='flex items-center gap-2 sm:gap-6 flex-wrap'>
                ...
                <li className='flex items-center gap-2 sm:gap-4 ml-auto flex-wrap justify-end'>
                    <div className='hidden sm:flex items-center border border-black rounded-2xl h-10 p-3 gap-2'>
                        ...
                        <input className='outline-none hidden md:block' type="text" name="Search" placeholder='Search Posts...' id="" />
                    </div>
                    <button onClick={() => navigate('/admin')} className='rounded-2xl cursor-pointer bg-sky-500 text-white p-2 text-xs sm:text-sm px-2 sm:px-4'>Admin</button>
                </li>
            </ul>
